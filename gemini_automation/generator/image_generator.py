...
def _wait_and_save(page, save_dir: Path, title: str = None, timeout: int = 180) -> str:
    ...
    while time.time() < deadline:
        ...
        if elapsed % 30 == 0 and elapsed > 0:
            page.screenshot(path=str(save_dir / f"debug_{elapsed}s.png"))
            print(f"Still waiting... ({elapsed}s elapsed)")

        try:
            btn = page.locator(download_btn_sel).first
            if btn.is_visible(timeout=1_000):
                print("Download button found. Saving image...")
                ...
                # Save to our output directory with the desired name
    ...
    page.screenshot(path=str(save_dir / "debug_timeout.png"))
    raise TimeoutError(f"Image generation timed out after {timeout} seconds.")


def _save_image(page, img_element, src: str, save_dir: Path, title: str = None) -> str | None:
    """Save image from src (data URI, blob URL, or element screenshot)."""
    stem = _sanitize(title) if title else f"gemini_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    filepath = save_dir / f"{stem}.png"

    try:
        if src.startswith("data:image"):
            _, encoded = src.split(",", 1)
            data = base64.b64decode(encoded)
            if len(data) < 10_000:   # too small, skip
                return None
            filepath.write_bytes(data)
            print(f"Saved (data URI): {filepath}")
            return str(filepath)

        if src.startswith("blob:") or (src.startswith("http") and "googleusercontent" in src):
            data = page.evaluate(
                """async (src) => {
            ...
            print(f"Saved (blob/url): {filepath}")
            return str(filepath)

        # Fallback: screenshot the element itself
        img_element.screenshot(path=str(filepath))
